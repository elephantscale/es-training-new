...
- The design process is also secure - not susceptible to exploits
- Application of the core engineering practice of _secure by design_
- Based on the standard engineering lifecycle or workflow
...
- Application of Engineering management to the lifecycle of a software product
...
- This module will focus only on Development
...
- Includes prioritized assessment of potential attackers
  - Individual hackers
  - Criminal or corporate attackers
...
- Many attacks stress system performance to put the target in an unstable state
- Nonfunctional requirements _must_ define the performance levels the system will experience in production
...
- Not a design - only describes what the end deliverable should be
...
- The IEEE _Best Practices for Software Requirements Specifications_ is the gold standard software specs
...
- If not part of the specification, a full suite of acceptance tests will also be developed
...
- The first line of design security is to prevent the insertion of malicious code during programming
- Three examples of malicious code insertion
  - Backdoors: Programmers insert code that allows access to the system for intruders
  - Logic Bombs: Code designed to compromise or damage the system when triggered by an attacker
  - Surprise Functionality: Functionality that is not in the spec that can be used by an attacker
- Malicious code insertion is generally cannot be identified by standard testing
  - White box testing that exercises all the code is one tool
  - Part of what testers do in exploratory testing
---
...
- 2012, a contractor for the US Army placed a logic bomb into the payroll systems for the US Army Reserves
  - The motivation was that his employer had lost the contract to build the system
...
- A security concern is that malicious programmers could insert backdoors that could be exploited later
...
- Use a VCS like git with audit capabilities
- Allows tracking of where all source code and changes originated
- Once code has been finalized, it is "frozen" and cannot be changed
- Only vetted code from the codebase is used in builds
- Code management and access policies enforced
- Standard code style so that everyone can read and understand the code

---

## Securing Writing the Code

- Pair programming is often used to improve security and code quality
...
- Extra layer of security, the code is reviewed by a third developer on a random basis
...
- All production code is also subject to formal code reviews and walk-throughs
- Multiple participants from both on the project and those not involved in the project
- Testers and security experts are often part of the review
- No code is 
